# YOUTUBE CLONE APP 🎬

> 유튜브 클론 코딩 연습

## Skills

- React.js
- Node.js
- MongoDB
- Redux

## Functions

- 유튜브 API를 이용한 검색 기능과, iframe 태그를 사용한 플레이어 삽입
- 직접 업로드한 비디오 리스트, 구독한 채널의 영상 리스트
- 비디오 업로드 구현 (React DropZone, multer, ffmpeg 사용)
- 채널 구독하기, 비디오 좋아요/싫어요 기능 구현
- 코멘트 기능 구현
- Redux을 이용한 유저 인증 로직을 위한 HOC 구현 (페이지 접근방지, 허용)
- 회원가입, 로그인, 비디오 업로드, 구독 추가 삭제, 리스트 API구현 (node.js + Mongoose)

### 데모 영상

        유튜브 API (리스트, 검색)

<img width="100%" src="https://user-images.githubusercontent.com/83004037/120109529-c511af80-c1a4-11eb-818a-535f9feda78a.PNG"/>

<img width="100%" src="https://user-images.githubusercontent.com/83004037/120110931-aa423980-c1aa-11eb-8c26-ee150d672023.PNG"/>

        비디오 업로드

<img width="100%" src="https://user-images.githubusercontent.com/83004037/120109969-af04ee80-c1a6-11eb-8710-26b02aefe09b.gif"/>

        구독 및 코멘트

<img width="100%" src="https://user-images.githubusercontent.com/83004037/120110495-b5946580-c1a8-11eb-97ed-0b4d901f2ef5.gif"/>

## 참고 사이트

- [유투브 클론 시리즈 by John Ann](https://www.youtube.com/playlist?list=PL9a7QRYt5fqnlSRu--re7N_1Ean5jFsh3)
